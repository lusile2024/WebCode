...
- [不兼容变更](#不兼容变更)
  - [卡片交互有效期变更](#卡片交互有效期变更)
  - [属性校验变更](#属性校验变更)
  - [JSON 全局结构和字段变更](#json-全局结构和字段变更)
  - [容器类组件布局属性默认值变更](#容器类组件布局属性默认值变更)
  - [vertical_spacing 和 horizontal_spacing 枚举值 & 映射数值变更](#vertical_spacing-和-horizontal_spacing-枚举值--映射数值变更)
  - [标题组件配置变更](#标题组件配置变更)
  - [图片组件不再支持通栏配置](#图片组件不再支持通栏配置)
  - [富文本（Markdown）组件废弃差异化跳转语法](#富文本markdown组件废弃差异化跳转语法)
  - [兜底高度 & 宽度变更](#兜底高度--宽度变更)
  - [废弃备注组件 & 交互模块](#废弃备注组件--交互模块)
- [新增属性和优化说明](#新增属性和优化说明)
  - [新增 streaming_mode 属性，支持流式更新](#新增-streaming_mode-属性支持流式更新)
  - [新增 element_id 属性，用于操作组件](#新增-element_id-属性用于操作组件)
  - [组件统一支持布局相关能力](#组件统一支持布局相关能力)
  - [富文本组件支持标准 markdown 语法](#富文本组件支持标准-markdown-语法)
  - [容器类组件新增可内嵌的组件类型](#容器类组件新增可内嵌的组件类型)

---

本文档介绍卡片 JSON 2.0 版本与 1.0 版本结构之间的不兼容变更和优化说明。了解完整的 JSON 2.0 结构数据，参考卡片 JSON 2.0 结构。
...
#### 结构变更
- JSON 2.0 版本新增 `body` 字段，`elements` 属性放置在 `body` 层级下。了解 2.0 整体结构，参考卡片 JSON 2.0 结构。
- JSON 2.0 版本不再支持通过 `i18n_elements` 字段设置全局多语言。你可通过 `i18n_content` 等局部多语言字段实现组件级别的多语言配置，详情参考配置卡片多语言。

#### fallback 字段变更
JSON 2.0 版本暂不支持使用 `fallback` 字段配置自定义的全局降级规则。

#### 默认值变更
JSON 2.0 版本中的 `update_multi` 默认值变更为 `true`，且暂时仅支持设为 `true`。`update_multi` 属性用于设置卡片是否为共享卡片：
- `true` 表示设置卡片为共享卡片，更新卡片的内容对所有收到这张卡片的人员可见
- `false` 表示设置卡片为独享卡片，更新卡片的内容对他人不可见
...
#### 表单容器
表单容器的 `vertical_spacing` 和 `horizontal_spacing` 字段的默认值由 16px 改为 12px，且支持开发者自定义配置。
...
#### 交互容器
交互容器的 `vertical_spacing` 和 `horizontal_spacing` 字段的默认值由 12px 改为 4px 和 8px，且支持开发者自定义配置。
...
#### 折叠面板
折叠面板的 `padding` 字段默认值变更：

**当配置了边框（border）或背景色（background_color）时：**
标题区 `padding` 字段的默认值变更为上下边距 4px，左右边距 8px。
...
**当未配置边框（border）或背景色（background_color）时：**
标题区 `padding` 字段的默认值变更为 0，内容区的内边距默认值变更为上边距 8px，右、下、左边距 0。
...
| 1.0 结构 | 2.0 结构 |
|---------|---------|
| `vertical_spacing` 和 `horizontal_spacing` 字段的枚举和对应的值为：<br><br>small：4px<br>medium：8px<br>large：16px | `vertical_spacing` 和 `horizontal_spacing` 字段的枚举和对应的值为：<br><br>small：4px<br>medium：8px<br>large：12px<br>extra_large：16px |
...
2.0 结构不再支持以下差异化跳转语法。你可使用 `<link></link>` 标签替代：
```html
<link icon='chat_outlined' url='https://applink.feishu.cn/client/chat/xxxxx' pc_url='' ios_url='' android_url=''> 战略研讨会 </link>
```
...
### 废弃备注组件 & 交互模块
2.0 结构不再支持备注（note）组件和交互模块（`"tag" 为 "action"`），相关效果可由以下组件和属性实现：
...
2.0 结构新增 `streaming_mode` 和 `summary` 字段，支持卡片流式更新、文本流式更新能力。详情参考流式更新 OpenAPI 调用指南。
...
卡片 JSON 2.0 结构支持除 HTMLBlock 外所有标准的 Markdown 语法和部分 HTML 语法。了解 Markdown 标准语法，请参考 CommonMark Spec 官方文档。你也可以使用 CommonMark playground 预览 Markdown 效果。了解更多，参考富文本（Markdown）。
...
<raw></raw>
<link></link>
<font></font> <!-- 支持嵌套其它标签，如 <font color=red>red<font color=green>green</font>again</font> -->
```
